...
| --- | --- |
| **Identity** | Profil utilisateur (`profiles`, préférences, `UserContext`, autorisations de coaching — cadence, silencieux, niveau d'intervention ADR §13), liaison Supabase Auth ↔ profil. **Writer unique de `user_context`** (règle AD-15, mapping `03-sync` § 4.2 : entité `UserContext` → module owner Identity, table locale `user_context`). **AD-17 v2 (thèmes, pack 05 §5) : le champ `user_context.theme` porte l'enum `AuroraTheme` (10 expressifs + 3 presets), PAS le binaire legacy `'light'|'dark'` ; `user_context.theme_style` porte `'light'|'dark'`. Le shape `AuroraTheme` et son migrateur (binaire → enum v2) vivent dans `packages/domain` (AD-15 SSoT) — Identity **consomme** l'enum, il ne la définit pas ; ce module est seulement le writer de la *valeur* persistée. |
...
- **Vague 0 (ce pack)** : `packages/domain` (SSoT AD-15, ratifié par la paire F-01), schéma SQL (AD-6/AD-15), policies RLS (§ 2.2), vues PowerSync (AD-7, owner `packages/data`), tables `job_queue`/`model_registry`, buckets R2 (noms concrets = données de vague 0), clés CI/CD (owner Foundation, AD-16c) + **gate de boundaries ESLint** (matrice AD-1/AD-2/AD-10/AD-15, owner Foundation AD-16c, détail SPEC § wave 0) + **greps CI vendor/secrets** (rappel § 2.3, `04-mobile` § 7.2e).
...
- Les **noms concrets** des buckets (structure **fixée** ici, valeurs = vague 0) ; le **mapping final** entité → package → équipe (AD-15) — précision : le mapping entité → module owner → table locale est déjà **frozen** dans `03-sync` § 4.2, il ne reste que la **colonne équipe** (SPEC OQ-02, réduite) ; le **ratification pnpm** (spine Open Question #1).
- **Vague 3 (option, non bloquante)** : si le team Agent a besoin d'un moteur de persistance de workflow multi-étapes pour la boucle du kernel (`Intent → … → Memory`, AD-12/F-09), celui-ci doit rester un **détail interne derrière `DispatcherApi` + le job store** (`job_queue`, § 5.2) : aucune source de déclenchement nouvelle (AD-8 : Supabase Cron + trigger Postgres uniquement), aucun changement de contrat côté client ; décision owner Foundation + Agent via ADR **additif** (famille admissible : moteurs de workflow persistés Postgres-backed, ex. DBOS — évaluation en vague 3, jamais en vague 0).
